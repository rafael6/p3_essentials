...
class bwDB:
    ...
    def getrec(self, id):
        ...
        '''
        query = 'SELECT * FROM {} WHERE id = ?'.format(self.table)  # column named id
    ...
    def update(self, id, rec):
        '''
            ...
            update a row in the table
                ...
                rec is a dict with key/value pairs corresponding to table schema
        '''
        klist = sorted(rec.keys())  # rec is  {'string': 'TWO'}, klist is  ['string']
        values = [ rec[v] for v in klist ]  # a list of values ordered by key; values is ['TWO']
        for i, k in enumerate(klist):       # don't udpate id
            if k == 'id':  # i is 0, k is 'string', is id 2
                del klist[i]  # delete 'string'
                del values[i]  # delete 'TWO'
...
def test():
    import os
    fn = ':memory:'     # file name; :: = in-memory database
    t = 'foo'           # table name
